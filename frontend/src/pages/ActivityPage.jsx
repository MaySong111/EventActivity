...
import { deleteActivity, getActivities } from "../http";
...
export default function ActivitiesPage() {
  ...
  //不设置初始值（data 默认 undefined）,那么后面的三元判断可以用!activities || isPending ?
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
                {activities.map((activity) => (
                  <ActivityCard
                    ...
                    onDelete={(id) => activities.filter((a) => a.id !== id)}
                  />
                ))}
              </Box>
            </Grid2>
